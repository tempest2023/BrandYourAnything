...
export function loadManagedAuctions() {
  ...
  const migrated = new Map<string, ManagedAuction>();
  for (const [auctionKey, managerKey] of [
    [LEGACY_MANAGED_LID_STORAGE_KEY, LEGACY_MANAGER_KEY_STORAGE_KEY],
    ["brand-anything-managed-auction", "brand-anything-auction-manager-key"],
  ]) {
    try {
      const raw = window.localStorage.getItem(auctionKey);
      const recoveryCode = window.localStorage.getItem(managerKey);
      if (!raw || !recoveryCode) continue;
      const entry = { ...JSON.parse(raw), recoveryCode };
      if (isManagedAuction(entry)) migrated.set(entry.slug, entry);
    } catch {
      // A malformed legacy entry must not hide other valid recovery codes.
    }
  }
  const entries = [...migrated.values()];
  // Do not persist on read: migration and storage events must not form a loop.
  return entries;
}
