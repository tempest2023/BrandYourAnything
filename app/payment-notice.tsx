...
const messages = {
  en: {
    ...
    failed: "We could not confirm your bid. Reload this page to check again before paying again.",
  },
  zh: {
    confirming: "正在确认付款…",
    accepted: "付款已确认，出价已记录。下方展示当前最高出价者。",
    refunded: "你的出价未能保持领先，押金已退还。",
    cancelled: "已取消付款，没有提交新的出价。",
    expired: "此付款链接已过期，请重新出价。",
    failed: "暂时无法确认出价。再次付款前，请刷新此页重试确认。",
  },
  es: {
    ...
    failed: "No pudimos confirmar tu puja. Recarga esta página antes de volver a pagar.",
  },
};
...
export function PaymentNotice({ state, retry }: { state: CheckoutReturnState; retry: () => void }) {
  ...
  return <div className={`payment-notice payment-notice--${state}`} role="status" aria-live="polite">
    ...
    {state === "failed" && <button type="button" onClick={retry}>{locale === "zh" ? "重试确认付款" : locale === "es" ? "Volver a comprobar el pago" : "Retry payment confirmation"}</button>}
  </div>;
}
