    ...
    <footer className={styles.footer}>
      <div className={styles.footerInner}>
        <div className={styles.footerBrand}>
          <Logo />
          <p>Rent out your laptop lid. Brands buy the sticker spots; you set the machine and the prices.</p>
        ...
        <nav className={styles.footerNav} aria-label="Footer">
          <div>
            <h2>Marketplace</h2>
            <Link href="/">All lids</Link>
            <Link href="/sell">Sell your lid</Link>
...
export function CreateLaptopForm() {
  ...
  }, [draftReady, extraNote, furthestStep, largePrice, layoutCount, listingDays, machine, machineCost, mediumPrice, ownership, screenSize, showcase, slug, smallPrice, specialPrice, specialSpot, step, stickerMonths, title]);
          ...
          <section className={styles.formPanel} aria-live="polite">
              ...
              <fieldset>
                ...
                <div className={styles.stackedCards}>
                  <button type="button" className={ownership === "own" ? styles.selectedCard : styles.optionCard} aria-pressed={ownership === "own"} onClick={() => setOwnership("own")}>
                    <strong>I own it</strong><span>The lid exists. What it raises is yours, and the page shows no goal to reach.</span>
                  </button>
            ...
            {step === 4 && (
              <fieldset>
                ...
                <div className={styles.priceList}>
                  <PriceField label={`${layoutCount === 10 ? 3 : 2} × Large`} dimensions="9.5 × 5.5 cm printed" value={largePrice} onChange={setLargePrice} />
                  <PriceField label={`${layoutCount === 10 ? 3 : 2} × Medium`} dimensions="9.5 × 4 cm printed" value={mediumPrice} onChange={setMediumPrice} />
                  <PriceField label={`${layoutCount === 10 ? 4 : 2} × Small`} dimensions="4.5 × 4.5 cm printed" value={smallPrice} onChange={setSmallPrice} />
                </div>
            ...
            {step === 5 && (
              <fieldset>
                ...
                <p className={styles.introCopy}>Long enough to be shared twice, short enough that a date on the page means something. Spots sell one at a time, so this is when the lid stops taking buyers rather than a finish line anybody races to.</p>
            ...
            {step === 6 && (
              <fieldset>
                <legend>How long do the stickers stay on?</legend>
                <p className={styles.introCopy}>This is what a buyer is actually buying, so it is yours to set rather than ours. Longer is worth more — and if you mean to sell the machine next year, do not promise two years of it.</p>
                ...
                <p className={styles.stickerNote}>It is shown on your listing and on the board, and it runs from the day of each purchase. Take one off early and the buyer is refunded for the time left.</p>
              </fieldset>
            )}
            ...
            {step < 7 && <div className={styles.actions}>{step > 0 && <button type="button" className={styles.backButton} onClick={backStep}>Back</button>}<button type="button" className={styles.continueButton} disabled={step === 1 && !machineIsValid} onClick={continueStep}>Continue</button></div>}
          </section>

          <aside className={styles.previewColumn} aria-label="Laptop lid preview">
