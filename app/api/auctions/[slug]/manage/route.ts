...
function authenticationError(error: unknown) {
  if (!(error instanceof PublishingAuthenticationError)) return null;
...
export async function PATCH(
  ...
  try {
    ...
    if (body.recoveryAction === "disable" || body.recoveryAction === "rotate") {
      const authOwner = await getPublishingOwnerCredential(request);
      if (!authOwner.ownerUserId) return Response.json({ error: "An authenticated account is required." }, { status: 401 });
