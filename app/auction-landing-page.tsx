        ...
        {spots.map((spot) => {
          ...
          return (
            <button
              ...
              aria-label={closed
                ? hasBid
                  ? t("home.closedHeldSpotAria", { id: spot.id, name: spotName, size: spot.size, holder: spot.holder, amount: money(spot.bid) })
                  : t("home.closedOpenSpotAria", { id: spot.id, name: spotName, size: spot.size })
                : hasBid
            ...
            >
              ...
              {(!hasBid || spot.logo) && <span className="lid-holder">{hasBid ? spot.holder : closed ? t("home.noWinner") : t("common.available")}</span>}
...
function BidDialog({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok && requiresPayment && payload.checkoutUrl) {
          ...
          {!submitted ? (
            <form onSubmit={handleSubmit}>
              ...
              <div className="deposit-box">
                ...
                <p className="due"><span>{requiresPayment ? t("home.stripeCheckout") : t("home.paymentIntegration")}</span><strong>{requiresPayment ? t("home.chargedNow") : t("home.notCharged")}</strong></p>
                <small>{requiresPayment ? t("home.stripeDepositNote") : t("home.depositNote")}</small>
              </div>
              ...
              {requiresPayment && !paymentsEnabled && <p className="bid-error" role="status">{t("home.paymentsNotReady")}</p>}
              <button className="primary-button bid-submit" type="submit" disabled={submitting || (requiresPayment && !paymentsEnabled)}>
                {submitting ? t("home.openingStripe") : requiresPayment ? t("home.continueStripe") : spot.bids > 0 ? `${t("common.outbid")} ${spot.holder}` : t("common.placeFirstBid")}
              </button>
...
export function AuctionLandingPage({ campaign, initialSnapshot }: AuctionLandingProps) {
  ...
  const bidEndpoint = campaign
    ...
    : "/api/bids";
  const isMac = !campaign
    || campaign.isDefault
    || (campaign.assetType === "laptop" && /^mac(?:book|\b)/i.test(campaign.laptopModel));
  ...
  const bidDialogRequestId = useRef(0);
  const [bidDialogRequest, setBidDialogRequest] = useState<{ spotId: number; requestId: number } | null>(null);
  ...
  const auctionClosed = campaign?.status === "closed" || countdown.closed;
  const selectedSpot = auctionClosed
    ? null
    : spots.find((spot) => spot.id === bidDialogRequest?.spotId) ?? null;
  ...
  const openBidDialog = useCallback((spotId: number) => {
    if (auctionClosed) return;
    bidDialogRequestId.current += 1;
    setBidDialogRequest({ spotId, requestId: bidDialogRequestId.current });
  }, [auctionClosed]);
    ...
    const check = async () => {
      ...
      try {
        ...
        if (payload.status === "accepted") {
          setPaymentNotice("accepted");
        } else if (payload.status === "refunded" || payload.status === "expired") {
          setPaymentNotice("refunded");
        } else if (attempt < 8) {
    ...
    <>
      ...
      <main id="main-content">
        <header className="hero" id="top">
          <div className={`live-visitors ${auctionClosed ? "live-visitors--closed" : ""}`}><span />{auctionClosed ? t("home.auctionClosed") : t("home.auctionOpen")}</div>
          <p className="total-visits"><span>·</span>{auctionClosed ? t("home.finalResults") : t("home.spotsAvailable", { count: availableSpotCount })}</p>
          <h1>{campaign?.title ?? t("home.heroTitle")}</h1>
          <p className="hero-subtitle">{campaign?.tagline ?? t("home.heroSubtitle")}</p>
          {paymentNotice !== "idle" && (
            <p className={`payment-notice payment-notice--${paymentNotice}`} role="status" aria-live="polite">
          ...
          <div className="funding">
            ...
            <p className="auction-time">{auctionClosed ? t("home.auctionClosedSummary") : `${t("home.auctionEnds", { countdown: countdown.label })} · ${filledSpotCount === 0 ? t("home.firstBrand") : t("home.stillOutbid")}`}</p>
          ...
          <div className="lid-view">
            <div className={`lid-layer lid-layer--live ${lidView === "live" ? "is-active" : ""}`} aria-hidden={lidView !== "live"}>
              <MacLid spots={spots} showApple={isMac} closed={auctionClosed} onSelect={(spot) => openBidDialog(spot.id)} />
            </div>
          ...
          <p className="lid-caption">{lidView === "live" ? auctionClosed ? t("home.closedCaption") : t("home.tapBid") : finalLookFailed ? t("home.incompleteHidden") : !finalLookReady ? t("home.appearWhenReady") : filledSpotCount === 0 ? t("home.cleanSlate") : t("home.finishedLid")}</p>
        ...
        <section className="auction-section" id="spots">
          <div className="section-inner auction-inner">
            <p className={`auction-status ${auctionClosed ? "auction-status--closed" : ""}`}><span />{auctionClosed ? t("home.auctionClosedStatus") : t("home.auctionStatus", { countdown: countdown.label, available: availableSpotCount, total: spots.length })}</p>
      ...
      <BidDialog
        key={bidDialogRequest?.requestId ?? "closed"}
        spot={selectedSpot}
        endpoint={bidEndpoint}
        requiresPayment={Boolean(campaign)}
        paymentsEnabled={campaign?.paymentsEnabled ?? false}
        onClose={() => setBidDialogRequest(null)}
